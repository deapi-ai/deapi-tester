  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex items-center justify-between px-4 py-1 border-b border-[var(--border-dim)] bg-[var(--surface-2)] flex-shrink-0">
        <span className="text-[10px] text-[var(--muted)]">{logs.length} events</span>
      ...
      <div ref={logContainerRef} className="flex-1 overflow-y-auto font-mono text-[11px]">
        {logs.length === 0 ? (
          <div className="p-8 text-xs text-[var(--text-faint)] text-center">No polling events yet</div>
        ) : (
          logs.map((log, idx) => (
            <details key={idx} className="group border-b border-[var(--border-dim)]">
              <summary className="flex items-center gap-2 px-4 py-1.5 cursor-pointer hover:bg-[var(--hover)] log-line">
                ...
                <span className="text-[var(--muted)] shrink-0">{log.endpointId}</span>
                <span className="text-[var(--text-faint)] shrink-0">#{log.attempt}</span>
              </summary>
              <div className="px-4 pb-2">
                <pre className="text-[10px] font-mono text-[var(--muted)] bg-[var(--surface)] rounded p-2 overflow-x-auto max-h-48">
