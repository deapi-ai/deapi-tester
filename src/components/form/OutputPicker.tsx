...
export function OutputPicker({ open, onClose, onPick }: OutputPickerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="bg-[var(--surface)] border border-[var(--border)] rounded-lg shadow-2xl w-[560px] max-h-[70vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
          <span className="text-sm font-medium text-[var(--text-emphasis)]">Pick from output</span>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-3">
          ...
          {!loading && !error && files.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-[var(--muted)]">
              ...
              <span className="text-xs text-[var(--text-faint)] mt-1">Run a generation and download the result first</span>
            </div>
          )}
            ...
            <div className="grid grid-cols-3 gap-2">
              {files.map((file) => (
                <button
                  ...
                  className="group relative flex flex-col bg-[var(--surface-2)] rounded border border-[var(--border)] hover:border-blue-500 transition-colors overflow-hidden disabled:opacity-50"
                >
                  {picking === file.name && (
